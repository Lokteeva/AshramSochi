import { useForm } from "react-hook-form"
import React from "react"
import style from "./form.module.css"
import axios from "axios";
import { Link } from "react-router-dom"


export default function Form (){


    const { register, handleSubmit, formState:{errors} } = useForm();
    const onSubmit = (data) => {
        axios.post(`https://6411825ce96e5254e2d79442.mockapi.io/Form`, data)
        alert("Спасибо! Ваша заявка отправлена. Мы скоро с Вами свяжемся!")
    }
    
    return(
        <div className={style.form}>
            <form id="form" onSubmit={handleSubmit(onSubmit)}>
            <h1>Заполните заявку на обратный звонок</h1>
            <div>
                <div > 
                     <input
                     {...register("lastName", {
                            required: true,
                            maxLength: 50,
                            pattern: /^[А-Яа-я]+$/i  
                        })}
                     name = "lastName"   
                     type="text"
                     className=""
                     placeholder="Фамилия"
                    />
                </div>
                {errors?.lastName?.type === "required" &&(
                    <p className="textRed">Поле Фамилия обязательно для заполнения</p>
                )}
                {errors?.lastName?.type === "maxLength" &&(
                    <p >Фамилия не может содержать более 50 символов</p>
                )}
                {errors?.lastName?.type === "pattern" &&(
                    <p >Поле заполненно некорректно</p>
                )}
                <div > 
                     <input
                     {...register("firstName", {
                            required: true,
                            maxLength: 50,
                            pattern: /^[А-Яа-я]+$/i 
                     })}
                     type="text"
                     className=""
                     placeholder="Имя"
                    />
                </div>
                {errors?.firstName?.type === "required" &&(
                    <p >Поле Имя обязательно для заполнения</p>
                )}
                {errors?.firstName?.type === "maxLength" &&(
                    <p className="textRed">Имя не может содержать более 50 символов</p>
                )}
                {errors?.firstName?.type === "pattern" &&(
                    <p >Поле заполненно некорректно</p>
                )}
                <div > 
                     <input
                     {...register("middleName", {
                            maxLength: 50,
                            pattern: /^[А-Яа-я]+$/i 
                     })}
                     type="text"
                     className=""
                     placeholder="Отчество"
                    />
                </div>
                {errors?.middleName?.type === "maxLength" &&(
                    <p >Отчество не может содержать более 50 символов</p>
                )}
                {errors?.middleName?.type === "pattern" &&(
                    <p >Поле заполненно некорректно</p>
                )}
                <div > 
                     <input
                     {...register("email", {
                            required: true,
                            maxLength: 50,
                            pattern: /^[A-Za-z0-9@._-]+$/i 
                     })}
                     name="email"
                     type="text"
                     className=""
                     placeholder="Email"
                   
                    />
                </div>
                {errors?.email?.type === "required" &&(
                    <p >Поле Email обязательно для заполнения</p>
                )}
                {errors?.email?.type === "maxLength" &&(
                    <p >Email не может содержать более 50 символов</p>
                )}
                {errors?.email?.type === "pattern" &&(
                    <p >Поле заполненно некорректно</p>
                )}
            

            <div > 
                     <input 
                     {...register("MobileNumber", {
                            required: true,
                            minLength: 6,
                            maxLength: 12,                            
                        })}
                     type="tel"
                     className=""
                     placeholder="Номер телефона"                   
                    />
                </div>
                {errors?.MobileNumber?.type === "required" &&(
                    <p >Поле Номер телефона обязательно для заполнения</p>
                )}
                {errors?.MobileNumber?.type === "maxLength" &&(
                    <p >Номер телефона не может содержать более 12 символов</p>
                )}
                {errors?.MobileNumber?.type === "minLength" &&(
                    <p>Номер телефона не может содержать менее 6 символов</p>
                )}
            </div>

            <div>
                <input  type="submit"/>
            </div>
            
            
            
    <Link exact to={'/'} >
      <button className="btnForm">На главную</button>
    </Link>
        </form>
    
        </div>
    )
}